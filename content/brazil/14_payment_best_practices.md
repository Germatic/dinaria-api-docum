...
## 2) PIX flow (do not assume success)

### QR scan is not confirmation
A PIX QR scan or redirect to `successUrl` **does not guarantee** that the payment is confirmed.

**Always confirm using:**
- Webhooks (**recommended**), or
- `GET /payments/{transactionId}` as a fallback (for delayed webhook delivery)

**Why:** PIX confirmation is asynchronous, repeated, or manipulated, and the payment may still fail or remain in progress.
